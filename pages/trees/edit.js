...
const Edit = () => {
  ...
  useEffect(() => {
    if (router) {
      const tree = treeStore.getOneTree(id)
      setFile(tree.image)
      setName(tree.name)
      setSciName(tree.sciName)
  ...
  const submitHandler = (event) => {
    ...
    treeStore.updateTrees(id, location)
    router.push("/trees")
  }

  const deleteLocation = () => {
    treeStore.deleteTrees(id)
    router.push("/trees")
  }
      ...
      {location && (
        <div className="p-3 space-y-5 bg-gray-100 rounded-lg">
          ...
          >
            ...
            <div
              className="grid grid-cols-3 w-full gap-5 px-2"
              onClick={() => {
                setTag([])
              }}
            >
              {tag.map((tag, idx) => (
                <div
                  key={idx}
                  className="bg-gradient-to-b from-green-300 to-blue-400 w-20 rounded-full text-center grid place-items-center scale-125"
                >
                  {tag}
                </div>
              ))}
